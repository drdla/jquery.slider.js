  ...
  Number.prototype.jSliderNice=function(iRoundBase) {
    ...
    if ((aMatches = sNum.match(re))) {
      var sIntPart = aMatches[2],
          iDecPart = (aMatches[4]) ? Number('0.' + aMatches[4]) : 0;
      if (iDecPart) {
        ...
        if (sNewDecPart.length < iRoundBase) {
          for (var i=0, iDiff=iRoundBase - sNewDecPart.length; i < iDiff; i++) {
            sDecPart = '0' + sDecPart;
          };
        }
        sDecPart = "," + sDecPart;
      ...
      if (Number(sIntPart) < 1000) {
        ...
        var sNewNum='';
        for (var i=1, l=sIntPart.length; i*3 < l; i++)
          sNewNum = sTSeparator + sIntPart.substring(l - i*3, l - (i-1) * 3) + sNewNum;
        sResult = sIntPart.substr(0, 3 - i*3 + l) + sNewNum + sDecPart;
      }
      if (aMatches[1])
        return '-'+sResult;
      else
